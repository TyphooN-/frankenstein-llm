...
qualification about to take the GPU out from under me?* That report is advisory. It
reads the router over loopback, the qualification's own durable state file, and the
...
# run_qualification writes one of these into "status". A step only holds the
...
# step ends -- only at the three whole-qualification exits -- and it does not exist at
...
RUNNING_QUALIFICATION_STATUSES = frozenset({
    "running", "running-with-failures", "running-with-blocked-admission",
})
PENDING_QUALIFICATION_STATUSES = frozenset({"starting", "waiting-artifacts", "waiting-safe-host"})
...
TERMINAL_QUALIFICATION_STATUSES = frozenset({
...
VERDICT_STEP_RUNNING = "qualification-step-running"
VERDICT_MAY_TAKE_GPU = "qualification-may-take-the-gpu"
VERDICT_IDLE_LAST_WRITE = "qualification-idle-per-last-write"
...
def load_supervisor(path: Path = QUALIFICATION_SUPERVISOR):
    """Import the qualification supervisor so its host classifier is reused, not copied.
    ...
    qualification -- the signal handlers are installed under ``__main__`` -- but it does
    parse ``HERMES_QUALIFICATION_QUIET_TIMEOUT`` eagerly and raises on a bad value, so
    every failure to load is caught and reported rather than crashing a read-only
    status command.
    """
    spec = importlib.util.spec_from_file_location("_qualification_supervisor", path)
...
def read_qualification_state(path: Path = QUALIFICATION_STATE) -> dict[str, Any]:
    """Classify the qualification's durable state without ever reading absence as safety.
    ...
    like an active qualification, because a reader that cannot tell has not been told
    the qualification is idle.
    """
    report: dict[str, Any] = {
        "state_path": str(path),
        "readable": False,
    ...
    if schema != "frankenstein-functional-qualification/1":
    ...
    if report["status"] in RUNNING_QUALIFICATION_STATUSES:
        report["disposition"] = "running"
    elif report["status"] in PENDING_QUALIFICATION_STATUSES:
        report["disposition"] = "pending"
    elif report["status"] in TERMINAL_QUALIFICATION_STATUSES:
...
def scan_host(supervisor: Any) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "blocking_conflicts": [],
        "advisory_conflicts": [],
        "blocking_reasons": sorted(supervisor.QUALIFICATION_BLOCKING_REASONS),
        "mem_available_bytes": None,
        "mem_floor_bytes": int(supervisor.MIN_AVAILABLE),
        "meets_qualification_memory_floor": None,
    }
    try:
        ...
        report["problem"] = f"{type(error).__name__}: {error}"
        return report
    blocking = supervisor.QUALIFICATION_BLOCKING_REASONS
    ...
    report.update({
        ...
        "meets_qualification_memory_floor": available >= supervisor.MIN_AVAILABLE,
    })
    return report
...
def collect_host_sharing(timeout: float = DEFAULT_TIMEOUT,
    ...
    Three verdicts, and none of them is "safe". A terminal qualification status earns
    only ``qualification-idle-per-last-write``, because the state file records what the
    ...
    state_path = QUALIFICATION_STATE if state_path is None else state_path
    reasons: list[str] = []
    try:
        ...
        # report: the qualification restarts it itself as a step.
        router = {"reachable": False, "problem": str(error), "endpoint": BASE_URL,
                  "loaded_models": [], "loads_models": False}

    qualification = read_qualification_state(state_path)
    try:
        ...
        host = {"scan": "unavailable", "problem": f"{type(error).__name__}: {error}",
                ...
                "mem_floor_bytes": None, "meets_qualification_memory_floor": None}

    if qualification["disposition"] == "running":
        verdict = VERDICT_STEP_RUNNING
        reasons.append(f"qualification status {qualification['status']!r} last recorded an executing step; "
                       "this is not live process proof")
    elif qualification["disposition"] == "pending":
        verdict = VERDICT_MAY_TAKE_GPU
        reasons.append(
            f"qualification status {qualification['status']!r} last recorded the supervisor "
            "working towards its next step; this is not live process proof")
    elif qualification["disposition"] == "terminal":
        verdict = VERDICT_IDLE_LAST_WRITE
        reasons.append(f"qualification status {qualification['status']!r} was its last durable write")
    else:
        verdict = VERDICT_MAY_TAKE_GPU
        reasons.append(
            "qualification state could not be classified"
            + (f" ({qualification['problem']})" if qualification["problem"] else "")
            + "; an unreadable or absent state is not evidence of an idle qualification")
    ...
    if host["scan"] != "ok":
        ...
        if host["blocking_conflicts"]:
            reasons.append(
                "processes the qualification would treat as blockers are running: "
                + ", ".join(sorted(host["blocking_counts_by_reason"])))
        if host["meets_qualification_memory_floor"] is False:
            reasons.append(
                f"MemAvailable {host['mem_available_bytes']} is below the qualification's "
    ...
    return {
        ...
        # qualification never consults it.
...
def render_host_sharing(report: dict[str, Any]) -> str:
    qualification, host, router = report["qualification"], report["host"], report["router"]
    lines = [
        f"Verdict: {report['verdict']} (advisory; not an admission guarantee)",
        f"Qualification: status={qualification['status'] or 'unknown'} "
        f"disposition={qualification['disposition']}"
        + (f" problem={qualification['problem']}" if qualification["problem"] else ""),
        f"  current_step={qualification['current_step'] or 'none'}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="advisory router + qualification + host report; reads only, and its exit "
