...
Native `llama-bench` throughput for the weight file `/home/typhoon/git/frankenstein-llm/models/gemma4-heretic/Gemma-4-12B-it-heretic-Q6_K.gguf`, served by this checkout as the `gemma4-heretic` compatibility alias.
...
| Compatibility alias | `gemma4-heretic` |
...
| Backends | ROCm |
| Native report directory | `logs/model-runs/benchmark-kk485fjb` |
...
```
/home/typhoon/git/frankenstein-llm/upstream/llama.cpp/build/bin/llama-bench -m /home/typhoon/git/frankenstein-llm/models/gemma4-heretic/Gemma-4-12B-it-heretic-Q6_K.gguf -dev ROCm0/ROCm1/ROCm2 -ts 1/0/1 -t 44 -ngl 999 -p 512 -n 128 -r 3 -o json
```
...
Averages and standard deviations are `llama-bench`'s own, read from the native JSON in `logs/model-runs/benchmark-kk485fjb/stdout.log`. Nothing here is derived from wall-clock timing of anything else.

| Test | Rate |
|---|---|
| Prompt processing (pp512) | 923.08 tok/s ± 30.17 |
| Token generation (tg128) | 24.13 tok/s ± 0.04 |

## Memory

- Sampled every 2.0 s across 19 readings while the benchmark ran; the figures below are peaks, not allocation requests.
  - `ROCm0` peak VRAM used: 4.94 GiB
  - `ROCm1` peak VRAM used: 0.55 GiB
  - `ROCm2` peak VRAM used: 7.50 GiB
  - lowest `MemAvailable` observed: 53.17 GiB
...
- `router-functional` verdict: **PASS** (recorded 2026-09-07T00:28:08-0400 on kernel `7.2.3-273-tkg-eevdf-llvm`).
  ...
  - required checks for this preset: `coherence`, `structured_output`
  - reader-only under candidate policy: no tools payload was sent, so no tool-call capability is claimed.
