...
Native `llama-bench` throughput for the weight file `/home/typhoon/git/frankenstein-llm/models/reranker/Qwen3-Reranker-8B-Q6_K.gguf`, served by this checkout as the `qwen3-reranker-8b` compatibility alias.

## Artifact

| Field | Value |
|---|---|
| Weight file | `Qwen3-Reranker-8B-Q6_K.gguf` |
| Full path | `/home/typhoon/git/frankenstein-llm/models/reranker/Qwen3-Reranker-8B-Q6_K.gguf` |
| Compatibility alias | `qwen3-reranker-8b` |
...
| Backends | ROCm |
| Native report directory | `logs/model-runs/benchmark-wffnj4ji` |
...
```
/home/typhoon/git/frankenstein-llm/upstream/llama.cpp/build/bin/llama-bench -m /home/typhoon/git/frankenstein-llm/models/reranker/Qwen3-Reranker-8B-Q6_K.gguf -dev ROCm0/ROCm1/ROCm2 -ts 1/0/0 -t 44 -ngl 999 -p 512 -n 128 -r 3 -o json
```
...
Averages and standard deviations are `llama-bench`'s own, read from the native JSON in `logs/model-runs/benchmark-wffnj4ji/stdout.log`. Nothing here is derived from wall-clock timing of anything else.

| Test | Rate |
|---|---|
| Prompt processing (pp512) | 1425.14 tok/s ± 63.96 |
| Token generation (tg128) | 65.31 tok/s ± 0.41 |

## Memory

- Sampled every 2.0 s across 10 readings while the benchmark ran; the figures below are peaks, not allocation requests.
  - `ROCm0` peak VRAM used: 6.49 GiB
  - `ROCm1` peak VRAM used: 0.55 GiB
  - `ROCm2` peak VRAM used: 1.99 GiB
  - lowest `MemAvailable` observed: 38.99 GiB
