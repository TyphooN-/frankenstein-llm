...
Native `llama-bench` throughput for the weight file `/home/typhoon/git/frankenstein-llm/models/Qwen3.8-27B-OBLITERATED-Q6_K.gguf`, served by this checkout as the `obliterated` compatibility alias.
...
| Compatibility alias | `obliterated` |
...
| Backends | ROCm |
| Native report directory | `logs/model-runs/benchmark-co2lusx8` |
...
```
/home/typhoon/git/frankenstein-llm/upstream/llama.cpp/build/bin/llama-bench -m /home/typhoon/git/frankenstein-llm/models/Qwen3.8-27B-OBLITERATED-Q6_K.gguf -dev ROCm0/ROCm1/ROCm2 -ts 1/1/1 -t 44 -ngl 999 -p 512 -n 128 -r 3 -o json
```
...
Averages and standard deviations are `llama-bench`'s own, read from the native JSON in `logs/model-runs/benchmark-co2lusx8/stdout.log`. Nothing here is derived from wall-clock timing of anything else.

| Test | Rate |
|---|---|
| Prompt processing (pp512) | 386.51 tok/s ± 9.05 |
| Token generation (tg128) | 13.66 tok/s ± 0.08 |
...
- Sampled every 2.0 s across 38 readings while the benchmark ran; the figures below are peaks, not allocation requests.
  ...
  - lowest `MemAvailable` observed: 37.13 GiB
