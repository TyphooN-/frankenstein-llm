...
Native `llama-bench` throughput for the weight file `/home/typhoon/git/frankenstein-llm/models/embedding/Qwen3-Embedding-8B-Q6_K.gguf`, served by this checkout as the `qwen3-embedding-8b` compatibility alias.

## Artifact

| Field | Value |
|---|---|
| Weight file | `Qwen3-Embedding-8B-Q6_K.gguf` |
| Full path | `/home/typhoon/git/frankenstein-llm/models/embedding/Qwen3-Embedding-8B-Q6_K.gguf` |
| Compatibility alias | `qwen3-embedding-8b` |
| Quantization (as the runtime read it) | `qwen3 8B Q6_K` |
| File size | 5.78 GiB |
| Parameters | 7.57 B |
...
| Backends | ROCm |
| Native report directory | `logs/model-runs/benchmark-37bnaq93` |
...
```
/home/typhoon/git/frankenstein-llm/upstream/llama.cpp/build/bin/llama-bench -m /home/typhoon/git/frankenstein-llm/models/embedding/Qwen3-Embedding-8B-Q6_K.gguf -dev ROCm0/ROCm1/ROCm2 -ts 1/0/0 -t 44 -ngl 999 -p 512 -n 128 -r 3 -o json
```
...
| Flash attention | `off` | `auto (runtime chooses)` |
| Context size | `4096` | sized to the benchmark workload, not to the serving context |

## Measured throughput

Averages and standard deviations are `llama-bench`'s own, read from the native JSON in `logs/model-runs/benchmark-37bnaq93/stdout.log`. Nothing here is derived from wall-clock timing of anything else.

| Test | Rate |
|---|---|
| Prompt processing (pp512) | 1430.10 tok/s ± 62.28 |
| Token generation (tg128) | 65.32 tok/s ± 0.42 |

## Memory

- Sampled every 2.0 s across 10 readings while the benchmark ran; the figures below are peaks, not allocation requests.
  - `ROCm0` peak VRAM used: 6.19 GiB
  - `ROCm1` peak VRAM used: 0.55 GiB
  - `ROCm2` peak VRAM used: 2.00 GiB
  - lowest `MemAvailable` observed: 39.14 GiB
...
- **Context:** the benchmark does not allocate this preset's serving context (`ctx-size = 4096`), so the residency of a served session is larger than anything measured here.
